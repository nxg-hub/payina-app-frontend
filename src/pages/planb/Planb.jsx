...
const PlanB = () => {
  ...
  useEffect(() => {
    // console.log('Location state received in PlanB:', JSON.stringify(location.state, null, 2));

    if (location.state) {
      setFormData(location.state);
      // console.log('Form data set in PlanB:', JSON.stringify(location.state, null, 2));
    } else {
      console.log('No state received in PlanB');
    }
  }, [location.state]);

  if (!formData) {
    // console.log('No form data available in PlanB');
    return (
      <section className="text-primary">
        <Navbar />
        <div className="container">
  ...
  const { planName, planPrice, planValidity, planData } = selectedPlan || {};
  ...
  const handleProceed = async () => {
    ...
    try {
      ...
      const initializePaymentResponse = await apiService.initializePayment(selectedPlan.id, email, amountInKobo);

      if (initializePaymentResponse.status === true && initializePaymentResponse.message === "Authorization URL created") {
        ...
        // Proceed to second API call using the reference from the first API
        const vendValueResponse = await apiService.vendValue(paymentReference);

        if (vendValueResponse.status === true) {
          // console.log('Transaction successful:', vendValueResponse.message);
    ...
    <section className="text-primary">
      ...
      <div className="container">
        ...
        <div className="flex px-5 gap-4">
          <p className="pt-8 pr-5 text-sm">Product</p>
          <div className="w-[42%] mt-24 h-1 ml-5 border-none bg-yellow"></div>
          <p className="pt-8 pl-5 text-sm">{planName || 'N/A'}</p>
        </div>
        <div className="flex px-5 gap-4">
          <p className="pt-8 pr-5 text-sm">Network</p>
          <div className="w-[42%] mt-24 ml-5 h-1 border-none bg-yellow"></div>
        ...
        <div className="flex justify-start gap-4">
          <p className="pt-8 pl-5 text-sm">Amount</p>
          <div className="w-[40%] mt-24 h-[1] border-none bg-yellow"></div>
          <p className="pt-14 text-sm">{planPrice || 'N/A'}</p>
        </div>
        <div className="flex gap-4">
          <p className="pt-8 pl-5 text-sm">Email</p>
          <div className="w-[40%] mt-24 h-1 border-none bg-yellow"></div>
          <p className="pt-14 text-sm">{email}</p>
        </div>
