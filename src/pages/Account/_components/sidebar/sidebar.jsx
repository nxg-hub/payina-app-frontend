...
export const Sidebar = () => {
  ...
  const userDetails = useSelector((state) => state.user.user);
  const reduxProfilePic = userDetails?.passportUrl;
  const reduxUserName = userDetails?.firstName;
  ...
  // Use Redux data first, fall back to API data
  const displayProfilePic = reduxProfilePic || userImage;
  const displayUserName = reduxUserName || apiUserName;
      ...
      <div className="space-y-[52px] flex flex-col w-full pb-20">
        <div className="flex flex-col justify-center items-center">
          <div className="mb-20">
            ...
            <div className="font-semibold text-xl mt-2">
              {error ? `Error: ${error}` : `Hi, ${displayUserName}`}
            </div>
            {(!bvn || bvn === '') && !nin && (
              <Link
                to="/verify"
