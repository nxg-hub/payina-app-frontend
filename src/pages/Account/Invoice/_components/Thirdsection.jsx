...
const Thirdsection = ({ clientId, showFiltered, filteredInvoices }) => {
    ...
    const fetchInvoices = async () => {
      try {
        ...
        if (clientId) {
          ...
          response = await fetch(`${import.meta.env.VITE_GET_CUSTOMERS_INVOICES_ENDPOINT}${corporateCustomerId}?page=0&size=10`, {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
            },
  ...
  const handleMarkAsPaid = async (invoiceId) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_MARK_AS_PAID_ENDPOINT}${invoiceId}/mark-as-paid`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleClickedInvoice = (invoice) => {
    ...
    setSelectedInvoice(newItem);
      ...
      <h2 className="text-xl font-bold mb-4"> {`Invoice preview for Invoice #${selectedInvoice.invoiceNumber}`}
      </h2>
      <p>
  ...
  <label className="relative inline-flex items-center cursor-pointer">
    ...
    <div
      className={`w-11 h-6 bg-blue-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-blue-400 rounded-full peer-checked:bg-[#A9A9A9] after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-bg-[#A9A9A9] after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-5 peer-checked:after:border-white ${
        invoice.paymentStatus === 'PAID' ? 'cursor-not-allowed  bg-[#A9A9A9]' : 'cursor-pointer'
      }`}
    ></div>
  </label>
</div>
