...
import { fetchLoanHistory } from '../../Redux/loanSlice';
const HistoryLoan = () => {
  const navigate = useNavigate();
  const id = useSelector((state) => state.loan.loanId);
  const loanName = useSelector((state) => state.loan.loanName);
  const loanHistory = useSelector((state) => state.loan.loanHistory);
  const loanHistoryLoading = useSelector((state) => state.loan.loanHistoryLoading);
  const [animateCards, setAnimateCards] = useState(false);
  const [newAuthToken] = useLocalStorage('newAuthToken', '');
  ...
  useEffect(() => {
    ...
    dispatch(fetchLoanHistory({ newAuthToken, id }));
  }, []);
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="space-y-3 sm:space-y-4 mt-11">
          {loanHistory.length === 0 ? (
            <p className="text-center text-sm "> No transaction history yet.</p>
          ) : (
            loanHistory.map((transaction, index) => {
              return (
                <div
                  key={transaction.id}
                  ...
                  <div className="relative bg-gradient-to-br from-white via-blue-50 to-blue-100 backdrop-blur-xl border border-white/10 hover:border-white/20 rounded-xl sm:rounded-2xl p-4 sm:p-6 transition-all duration-300 hover:scale-[1.01] sm:hover:scale-[1.02]">
                    <div className="flex sm:flex-row sm:items-center justify-between gap-4">
                      ...
                      <div className="text-right flex-shrink-0 self-start sm:self-center">
                        <div className={`text-md sm:text-xl font-bold ${'text-green-400'}`}>
                          {transaction.amount > 0 ? '+' : ''}₦
                          {Math.abs(transaction.amountPaid).toFixed(2)}
                        </div>
