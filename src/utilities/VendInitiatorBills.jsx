...
const VendInitiatorBills = ({
  ...
  const handleBillPayment = async () => {
    ...
    try {
      ...
      const billPaymentResponse = await axios.post(import.meta.env.VITE_PAY_BILL_ENDPOINT, billData, {
        headers: {
          'Content-Type': 'application/json',
          Accept: '*/*',
          Authorization: `Bearer ${newAuthToken}`,
      ...
      if (billPaymentResponse.data.status === 'success') {
        ...
        const debugMessage = billPaymentResponse.data.debugMessage || `Bill payment failed: ${errorNarration}`;
